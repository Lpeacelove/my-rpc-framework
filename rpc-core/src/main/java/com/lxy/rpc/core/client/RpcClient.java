...
public class RpcClient {
    ...
    public synchronized void connect() {
        ...
        if (channel != null && channel.isActive()) {
            logger.info("[RpcClient] is already connected to {}:{}", host, port);
            return;
        }
        ...
        try {
            ...
            if (channelFuture.isSuccess()) {
                ...
                logger.info("[RpcClient] connected to {}:{}", host, port);
                // 为该channel设置一个监听器，当该channel关闭时，会触发该监听器并执行相应的操作。
                channel.closeFuture().addListener(future -> {
                    logger.info("[RpcClient] disconnected from {}:{}", host, port);
                    ...
                    PENDING_RPC_FUTURES.forEach((id, pendingFuture) -> {
                        if (!pendingFuture.isDone()) {
                            logger.warn("[RpcClient] Pending future for request ID: {} is not done.", id);
                            pendingFuture.completeExceptionally(new RpcNetworkException("Connection closed before response for request ID: " + id));
                        }
                    });
                ...
                logger.error("[RpcClient] failed to connect to {}:{}. Cause: {}", host, port, channelFuture.cause().getMessage());
            }

        } catch (Exception e) {
            logger.error("[RpcClient] failed to initiate connect to {}:{}. Cause: {}", host, port, e.getMessage());
            if (e instanceof InterruptedException) {
                Thread.currentThread().interrupt(); // 重新设置中断状态
            }
            throw new RpcNetworkException("[RpcClient] " + RpcErrorMessages.format(RpcErrorMessages.CONNECT_FAILED, host+":"+port));
        }
    }
    ...
    public CompletableFuture<RpcMessage> sendRequest(RpcMessage message) {
        // 判断当前channel状态
        if (channel == null || !channel.isActive()) {
            logger.warn("[RpcClient] is not connected. Cannot send request ID: {}", message.getHeader().getRequestID());
            // 返回一个错误的异步结果
            return CompletableFuture.failedFuture(new RpcNetworkException("[RpcClient] " +
                    RpcErrorMessages.format(RpcErrorMessages.CONNECT_FAILED, message.getHeader().getRequestID())));
        }
        ...
        logger.info("[RpcClient] is sending request ID: {} to {}", message.getHeader().getRequestID(), this.channel.remoteAddress());
        ...
        this.channel.writeAndFlush(message).addListener((ChannelFutureListener) channelFuture -> {
            if (!channelFuture.isSuccess()) {
                // 发送失败
                logger.error("[RpcClient] failed to send request ID: {} to {}. Cause: {}",
                        message.getHeader().getRequestID(), this.channel.remoteAddress(), channelFuture.cause().getMessage());
                // 移除该异步响应对象，并设置一个失败的CompletableFuture
                PENDING_RPC_FUTURES.remove(message.getHeader().getRequestID());
                future.completeExceptionally(channelFuture.cause());
            } else {
                // 发送成功
                logger.info("[RpcClient] sent request ID: {} to {}",
                        message.getHeader().getRequestID(), this.channel.remoteAddress());
            }
        });
        return future;
    }
    ...
    public void close() {
        if (channel != null && channel.isOpen()) {
            try {
                logger.info("[RpcClient] closing connection to {}:{}...", host, port);
                channel.close().syncUninterruptibly(); // 关闭连接，并等待关闭操作完成
            } catch (Exception e) {
                logger.error("[RpcClient] failed to close connection to {}:{}. Cause: {}", host, port, e.getMessage());
            }
        }
        if (eventLoopGroup != null && !eventLoopGroup.isShutdown()) {
            eventLoopGroup.shutdownGracefully().syncUninterruptibly(); // 关闭EventLoopGroup，并等待关闭操作完成
            logger.info("[RpcClient] closed connection to {}:{}", host, port);
        }
